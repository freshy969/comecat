import * as config from '../config';
import { EN } from '../const';

export const SidebarTitle1 = {
    EN: "Chat"
}

export const SidebarTitle2 = {
    EN: "User Settings"
}

export const SidebarTitle3 = {
    EN: "Links"
}

export const SidebarChat = {
    EN: "Chat"
}

export const SidebarHistory = {
    EN: "History"
}

export const SidebarSearch = {
    EN: "Search"
}

export const SidebarFavorite = {
    EN: "Favorite"
}


export const SidebarNewRoom = {
    EN: "New Room"
}

export const SidebarMarkAll = {
    EN: "Mark All"
}

export const SidebarProfile = {
    EN: "Edit Profile"
}

export const SidebarPassword = {
    EN: "Change Password"
}

export const SidebarAdmin = {
    EN: "Admin Console"
}

export const SidebarLogout = {
    EN: "Logout"
}

export const SidebarHelp = {
    EN: "Help"
}

export const SidebarTitleIntegration = {
    EN: "Integrations"
}

export const SidebarConnectors = {
    EN: "Connectors"
}

export const SidebarWebhookAPI = {
    EN: "API/WebHooks"
}

export const SidebarBots = {
    EN: "ChatBots"
}

export const SidebarSettings = {
    EN: "Settings"
}

export const SidebarOfficialSupport = {
    EN: "User Support"
}