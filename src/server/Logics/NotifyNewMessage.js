...
var SendToSocial = require("./SendToSocial");
var SendToChatbot = require("./SendToChatbot");
        ...
        async.waterfall([
            ...
            function (result, done) {
                ...
                userModel.findOne({
                    ...
                    if (obj.room.external && result.sender.isGuest != 1)
                        SendToSocial.send(obj.room, obj, originalRequestData);

                    if (obj.room.bot && result.sender.isGuest == 1)
                        SendToChatbot.send(obj.room, obj);

                    // disable chatbot if agent answers
                    if (
                        result.sender._id != Config.robotUserId &&
                        result.sender.isGuest != 1 &&
                        obj.room.bot &&
                        obj.room.bot.botEnabled) {
            ...
            function (result, done) {
                ...
                if (result.room) {
                    const userIds = result.room.users;
                    criteria["userId"] = { $in: userIds };
                }

                webHookModel.find(criteria, (err, webhooks) => {
                    ...
                    const requestbody = {
                        ...
                        sender:
                        {
                            ...
                            isGuest: obj.user.isGuest == 1 ? 1 : 0
                        }
                    };
                    ...
                    webhooks.forEach((webhook) => {
                        ...
                        request.post({
                            ...
                            console.log(' webhook sent', requestbody);
