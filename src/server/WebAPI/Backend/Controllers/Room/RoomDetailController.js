 ...
 **/
	...
	router.get('/:roomId', tokenChecker, function (request, response) {
		...
		function (result, done) {
			...
			if (_.isArray(usersLimited)) {
				...
				userModel.find({
					...
					result.room.userModels = userFindResult.map((row) => {
						return row.toObject();
					});
		...
		function (result, done) {
			...
			// check bot status
			if (result.room.external &&
				result.room.external.serviceSettingId) {

				connectorModel.findOne({
					_id: result.room.external.serviceSettingId
				}, function (err, connectorFindResult) {

					if (connectorFindResult)
						result.connector = connectorFindResult.toObject();
		...
		function (result, done) {
			...
			if (result.connector) {
				...
				if (result.connector.botId) {
					result.room.botAvailable = true;

					if (result.room.bot)
						result.room.botEnabled = result.room.bot.botEnabled;
					...
					userModel.findOne({
						...
						if (userFindResult)
							result.room.botUser = userFindResult.toObject();

						done(null, result);
					});

				} else {
					result.room.botAvailable = false;
				}
