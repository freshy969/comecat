...
(function (global) {
    ...
    function sendEmail(to, type, params, cb) {
        ...
        const from = 'Come.Cat Support <support@come.cat>';
        let source = "";
        let subject = "";

        if (type == Const.emailTypeSignUp) {

            const templatePath = path.resolve(__filename, '../../EmailTamplates/signup.hbs');
            source = fs.readFileSync(templatePath).toString();
            subject = "[come.cat] Check Activation Code";

        }

        if (type == Const.emailTypeForget) {

            const templatePath = path.resolve(__filename, '../../EmailTamplates/forget.hbs');
            source = fs.readFileSync(templatePath).toString();
            subject = "[come.cat] Reset Password";
